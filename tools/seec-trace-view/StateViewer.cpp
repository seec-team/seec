...
bool StateViewerPanel::Create(wxWindow *Parent,
  ...
  // Create the notebook that holds the thread states.
...
void StateViewerPanel::show(OpenTrace &TraceInfo,
  ...
  // StateTree->setRoot(TraceInfo, *(State.getThreadStates()[0]));
  ...
  else if (ThreadStateModels.size() > Threads.size()) {
    // TODO.
  }
  
  // Update the thread state models.
  for (std::size_t i = 0; i < Threads.size(); ++i) {
    ThreadStateModels[i]->setRoot(TraceInfo, *Threads[i]);
  }
